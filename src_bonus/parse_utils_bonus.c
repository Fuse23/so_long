...
/*   Updated: 2022/05/14 15:05:03 by falarm           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_mapdata2(t_mapdata *mapdata, int i)
{
...
t_mapdata	*init_mapdata(char *line)
{
	...
	while (mapdata->map[i])
		i++;
	init_mapdata2(mapdata, i);
	free(line);
	return (mapdata);
}

void	init_sprites3(t_mapdata *mapdata, int x, int y)
{
	...
	if (!mapdata->sprites.exit1 || !mapdata->sprites.collectible
...
void	init_sprites2(t_mapdata *mapdata, int x, int y)
{
	...
	if (!mapdata->sprites.wall || !mapdata->sprites.floor
		|| !mapdata->sprites.player1 || !mapdata->sprites.exit0
		|| !mapdata->sprites.player2)
	{
		ft_putendl_fd("Error: can't open xpm file", 2);
		exit(EXIT_FAILURE);
	}
	init_sprites3(mapdata, x, y);
}
